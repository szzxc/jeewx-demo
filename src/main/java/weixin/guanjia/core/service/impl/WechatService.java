...
import weixin.guanjia.core.util.*;
...
public class WechatService {
    ...
    String doDingYueEventResponse(Map<String, String> requestMap, TextMessageResp textMessage, ResourceBundle bundler, String respMessage
        ...
        // 存数据库
        WeixinUserEntity userTmp =  weixinUserService.obtainWeixinUserInfo(fromUserName);
        WeixinUserEntity user = weixinUserService.findBindByOpenId(fromUserName);
        if (user != null) {//以前关注过
            user.setCity(userTmp.getCity());
            user.setCountry(userTmp.getCountry());
            user.setGroupid(userTmp.getGroupid());
            user.setHeadimgurl(userTmp.getHeadimgurl());
            user.setLanguage(userTmp.getLanguage());
            user.setNickname(userTmp.getNickname());
            user.setOpenid(userTmp.getOpenid());
            user.setProvince(userTmp.getProvince());
            user.setRemark(userTmp.getRemark());
            user.setSex(userTmp.getSex());
            user.setSubscribe(userTmp.getSubscribe());
            user.setSubscribe_time(userTmp.getSubscribe_time());
            user.setTagid_list(userTmp.getTagid_list());
            user.setUserId(userTmp.getUserId());
            user.setWechatBindTime(Calendar.getInstance().getTime());
            user.setWechatUrlToken(DigestUtils.md5Hex(user.getOpenid()));
            ...
            userTmp.setWechatBindTime(Calendar.getInstance().getTime());
            userTmp.setWechatUrlToken(DigestUtils.md5Hex(userTmp.getOpenid()));
            systemService.save(userTmp);
        }
        return respMessage;
    }
    ...
    public void flushToken() throws IOException {
        String ret = HttpRequestUtil.executeRquest(WeChatSetting.ACCESS_TOKEN_URL(), null, HttpRequestUtil.GET,
        ...
        LOG.info("access_token:" + retStr);
        WeChatSetting.setACCESS_TOKEN(retStr);

//        WeixinAccountEntity account = weixinAccountService.findLoginWeixinAccount();
//        account.setAccountaccesstoken(retStr);
//        // 重置事件
//        account.setAddtoekntime(new Date());
//        weixinAccountService.saveOrUpdate(account);
    }

    public WeChatStatusCode.GlobalStatus executeStatus(String msg) throws IOException {
        WeChatStatusCode.GlobalStatus status;
        ...
        if (errcode == null) {
            status = WeChatStatusCode.GlobalStatus.SUCCESS;
        } else {
            status = WeChatStatusCode.GlobalStatus.valueOf(errcode);
            if (WeChatStatusCode.GlobalStatus.TOKEN_ERROR.equals(status)
                    || WeChatStatusCode.GlobalStatus.PARAMETER_ACCESSTOKEN_TIMEOUT.equals(status)) {// token 超时，需要刷新token
                flushToken();
            }
        }
        return status;
